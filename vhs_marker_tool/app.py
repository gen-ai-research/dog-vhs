...
############## START : TO CHECK DUPLICATE IMAGES ######################

# # Folder paths
# IMAGE_FOLDER_DUP = os.path.abspath("../data-prep/Report_generation/Train_Images")
# DUPLICATES_FOLDER = os.path.abspath("../data-prep/Report_generation/duplicates")


# def get_sorted_images():
#     """Fetch images ending with '_3.png', remove '_3.png', sort numerically, and restore filenames."""
#     images = [f for f in os.listdir(IMAGE_FOLDER_DUP) if f.lower().endswith("_3.png")]

#     # Remove "_3.png", extract numeric part, and sort
#     def extract_base_number(filename):
#         base_name = filename.replace("_3.png", "")  # Remove '_3.png'
#         return int(base_name) if base_name.isdigit() else float('inf')  # Sort numerically

#     sorted_images = sorted(images, key=extract_base_number)  # Sort numerically

#     return sorted_images

# #image_files = get_sorted_images()

# #print(image_files[0:100])  # Debugging to check the correct order
...
# # Create the duplicates folder if not exists
# os.makedirs(DUPLICATES_FOLDER, exist_ok=True)

# # # Get all images ending with "_3"
# # potential_duplicates = [f for f in os.listdir(IMAGE_FOLDER_DUP) if f.endswith("_3.png") or f.endswith("_3.jpg") or f.endswith("_3.jpeg")]
# # potential_duplicates.sort()  # Optional: Sort files

# # # Remove "_3.png", sort numerically, and restore filenames
# # def extract_base_number(filename):
# #     base_name = filename.replace("_3.png", "")  # Remove '_3.png'
# #     return int(base_name) if base_name.isdigit() else float('inf')  # Sort numerically

# # # Sort potential duplicates numerically
# # potential_duplicates.sort(key=extract_base_number)

# # # Mapping original images without "_3"
# # image_pairs = []
# # for img in potential_duplicates:
# #     base_name = img.rsplit("_3", 1)[0]  # Remove "_3"
# #     for ext in [".png", ".jpg", ".jpeg"]:
# #         counterpart = base_name + ext
# #         if counterpart in os.listdir(IMAGE_FOLDER_DUP):
# #             image_pairs.append((counterpart, img))  # (Original, Duplicate)
# #             break

# # print(image_pairs[0:10])  # Debugging to check correct mapping

# def find_original_name(duplicate_name):
#     """Find the corresponding original image by removing '_3' from the name."""
#     original_name = duplicate_name.replace("_3", "")
#     if original_name in os.listdir(IMAGE_FOLDER_DUP):
...
# @app.route("/get_images/<int:index>")
# def get_images(index):
#     """ Get image pair based on index """
#     if 0 <= index < len(image_pairs):
#         return jsonify({
#             "index": index,
#             "total": len(image_pairs),
#             "original": image_pairs[index][0],
#             "duplicate": image_pairs[index][1]
#         })
#     return jsonify({"error": "Index out of range"}), 404

# @app.route("/get_images_by_name/<string:image_name>")
# def get_images_by_name(image_name):
#     """Fetch images based on manually entered name."""
#     duplicate_name = image_name.replace(".png", "_3.png")  # Ensure it's checking the duplicate version

#     if duplicate_name in image_files:
#         original_name = find_original_name(duplicate_name)

#         if original_name:
#             index = image_files.index(duplicate_name)  # Find its index in the list
#             return jsonify({
...
# @app.route("/mark_duplicate", methods=["POST"])
# def mark_duplicate():
#     """ Move duplicate image to 'duplicates' folder """
#     data = request.json
#     duplicate_name = data.get("duplicate")

#     LOG_FILE = "duplicates.txt"

#     if duplicate_name:
#         src_path = os.path.join(IMAGE_FOLDER_DUP, duplicate_name)
#         dest_path = os.path.join(DUPLICATES_FOLDER, duplicate_name)
#         if os.path.exists(src_path):
#             shutil.move(src_path, dest_path)
#             with open(LOG_FILE, "a") as log_file:
#                 log_file.write(f"{duplicate_name}\n")  # Log the moved file
#             return jsonify({"message": f"Moved {duplicate_name} to duplicates"})
...
#     """ Serve images """
#     return send_from_directory(IMAGE_FOLDER_DUP, filename)

# @app.route("/mark_incorrect", methods=["POST"])
# def mark_incorrect():
#     """Move both original and duplicate images to the 'incorrect' folder."""
#     data = request.json
#     duplicate_name = data.get("duplicate")
#     original_name = data.get("original")

#     INCORRECT_FOLDER = os.path.abspath("../../data-prep/Report_generation/incorrect") 

#     os.makedirs(INCORRECT_FOLDER, exist_ok=True)

#     LOG_FILE = "incorrect.txt"
#     os.makedirs(INCORRECT_FOLDER, exist_ok=True)

#     if duplicate_name and original_name:
#         src_duplicate = os.path.join(IMAGE_FOLDER_DUP, duplicate_name)
#         src_original = os.path.join(IMAGE_FOLDER_DUP, original_name)
#         dest_duplicate = os.path.join(INCORRECT_FOLDER, duplicate_name)
#         dest_original = os.path.join(INCORRECT_FOLDER, original_name)

#         try:
#             shutil.move(src_duplicate, dest_duplicate)
#             shutil.move(src_original, dest_original)
#             image_files.remove(duplicate_name)  # Remove from the list
#             image_pairs[:] = [(orig, dup) for orig, dup in image_pairs if orig != original_name and dup != duplicate_name]
#             with open(LOG_FILE, "a") as log_file:
#                 log_file.write(f"{original_name}, {duplicate_name}\n")  # Log both files
#             return jsonify({"message": f"{original_name} and {duplicate_name} moved to incorrect folder."})
#         except Exception as e:
#             return jsonify({"error": f"Failed to move images: {str(e)}"})

#     return jsonify({"error": "Images not found"})

# DELETED_FOLDER = os.path.abspath("../../data-prep/Report_generation/deleted_images") 

# # Ensure deleted_images folder exists
# os.makedirs(DELETED_FOLDER, exist_ok=True)

# DELETED_LOG_FILE = "../../data-prep/Report_generation/deleted_images_log.txt"  # Dedicated log file
...
#     original_path = os.path.join(IMAGE_FOLDER_DUP, image_name)
#     deleted_path = os.path.join(DELETED_FOLDER, image_name)

#     if os.path.exists(original_path):
#         shutil.move(original_path, deleted_path)
        
#         # Append deleted file path to log file
#         with open(DELETED_LOG_FILE, "a") as log_file:
#             log_file.write(f"{os.path.abspath(deleted_path)}\n")

#         return jsonify({
#             "success": f"{image_name} moved to deleted_images folder.",
#             "deleted_file_path": os.path.abspath(deleted_path)
#         })
#     else:
#         return jsonify({"error": "Image not found."}), 404
